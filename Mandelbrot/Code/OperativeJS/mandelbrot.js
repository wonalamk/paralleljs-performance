...
function initWorkers() {
  //Inicjacja workerów i nadanie im pracy
  for (var i = 0; i < NUMBER_OF_WORKERS; i++) {
    var worker = operative({
      process: calculateRow,
    });
...
function startWorkers() {
  ...
  for (var i = 0; i < NUMBER_OF_WORKERS; i++) {
    ...
    if (worker.idle) {
      ...
      worker.process(task, function (data) {
...
function saveToCSV(data) {
  ...
  pom.setAttribute(
    "download",
    `OperativeJS ${BROWSER}-${NUMBER_OF_WORKERS}-${MAX_ITERATIONS}-${ZOOM}.csv`
  );
  pom.click();
}
