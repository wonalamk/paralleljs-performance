...
function renderElements(img) {
    ...
    var p = new Parallel(tasks, options={ evalPath: './eval.js', synchronous: false, maxWorkers: NUMBER_OF_WORKERS});
    p.require('color-thief.js')
    p.require('quantize.js');
    p.map(runWorker).then(function (data) {
        console.log(data)
        for (var i = 0; i < data.length; i++) {
            draw(data[i]);
        }
...
function saveToCSV(data) {
    ...
    pom.setAttribute(
        "download",
        `ParallelJS-${BROWSER}-${TEST_IMAGE}-${NUMBER_OF_WORKERS}-${PIXEL_SIZE}.csv`
    );
    pom.click();
}